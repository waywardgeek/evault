			...
			c.JSON(http.StatusOK, gin.H{
				...
				"endpoints": gin.H{
					...
					"protected": gin.H{
						...
						"GET /api/user/info":          "Get detailed user account info (requires auth)",
						"POST /api/user/refresh":      "Refresh JWT token (requires auth)",
						"POST /api/user/update-email": "Update user email (requires auth)",
						"DELETE /api/user/delete":     "Delete user account (requires auth)",
